...
export async function search({ q }) {
  // +++ SECURITY IMPROVEMENT: Build the URL from parts +++
  const apiKey = process.env.AWIN_API_KEY; // Use the new environment variable
  ...
  // Construct the base URL. This is much cleaner and safer.
  ...
  // +++ Add the search query to the URL to make it specific and avoid the long URL 400 error +++
  const urlObj = new URL(baseFeedUrl);
  // The 'query' parameter tells the Awin server to filter results before sending them
  urlObj.searchParams.set('query', q || '');

  const targetUrl = urlObj.toString();
  console.log("Built Travis Perkins feed URL:", targetUrl); // This will log the new, shorter URL

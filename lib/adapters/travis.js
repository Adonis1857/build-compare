...
export async function search(keywords) {
  // Use whatever environment variable name you KNOW works
  const csvUrl = process.env.TRAVIS_CSV_URL || process.env.TRAVIS_PERKINS_CSV_URL || process.env.TP_CSV_URL;
  if (!csvUrl) {
    console.warn("Missing Travis CSV URL env var");
    return [];
  }
...
function mapRowToOffer(r, keywords) {
  ...
  const url = r["merchant_deep_link"] || ""; // Use the direct link for now
  ...
  return {
    ...
    product: name,
    price,
    url, // This will use the direct merchant link
    image,
    unit: "per item",
    pack: r["size"] || "",

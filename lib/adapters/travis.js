...
/**
 * Travis Perkins via AWIN Create-a-Feed (CSV).
 * Uses FEED_TRAVIS_URL (required), FEED_TRAVIS_AUTH (optional).
 */
...
function buildUrl(raw, base, aff) {
  ...
  const b = process.env.NEXT_PUBLIC_TP_BASE || "https://www.travisperkins.co.uk";
  let u; try { u = new URL(raw || "/", base || b); } catch { return raw || ""; }
...
export async function search({ q }) {
  ...
  for (const r of rows) {
    ...
    const url = buildUrl(deeplink || merchUrl);

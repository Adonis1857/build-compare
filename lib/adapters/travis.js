...
export async function search({ q }) {
  // 1. Get the PRECISE URL from the environment variable
  ...
  let res;
  try {
    // 2. Fetch the EXACT URL provided by Awin. DO NOT CHANGE IT.
  ...
  try {
    ...
    const offers = [];
    // 5. Loop through every product and check for matches
    for (const r of rows) {
      ...
      let   url   = r["merchant_deep_link"] || deeplink; // Use 'let' instead of 'const' because we will change it

      // +++ NEW LINE: This fixes the "URL invalid" error! +++
      // It ensures the URL is properly encoded for web browsers
      url = encodeURI(url);

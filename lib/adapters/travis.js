// lib/adapters/travis.js
import { fetchWithTimeout } from "./utils";
import zlib from "node:zlib";

/**
 * Travis Perkins via AWIN Create-a-Feed (CSV, sometimes gzipped without headers)
 * Env:
 *  - FEED_TRAVIS_URL (required)
 *  - FEED_TRAVIS_AUTH (optional)
 *  - NEXT_PUBLIC_TP_BASE (optional)
 *  - TP_AFFIL_LINK (optional)
 */
...
function paramsFrom(link){ try { return link ? new URL(link).searchParams : null; } catch { return null; } }
function buildUrl(raw, base, aff) {
  if (raw && /^https?:\/\//i.test(raw)) return raw; // already absolute (deeplink)
...
export async function search({ q }) {
  ...
  // Fetch as bytes so we can gunzip if needed
